...
class CronJob(Model):
    ...
    async def mark_running(self):
        ...
        logger.info("mark_running(%s)", self.id)
        self.status = self.Status.RUNNING
        await self.asave()

    async def mark_completed(self):
        """Mark this job as completed and calculate next run time."""
        logger.info("mark_completed(%s)", self.id)
        self.status = self.Status.COMPLETED
        self.last_run = _now()

        # Simple scheduling - just add 1 minute for next run
        # In a real system, you'd parse the cron expression
        self.next_run = _now() + dt.timedelta(minutes=1)
